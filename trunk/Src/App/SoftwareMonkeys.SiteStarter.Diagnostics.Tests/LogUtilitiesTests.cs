...
namespace SoftwareMonkeys.SiteStarter.Diagnostics.Tests
{
	...
	public class LogUtilitiesTests : BaseDiagnosticsTestFixture
	{
		...
		public void Test_AnalyzeLog()
		{
			...
			string testLogDir = Path.GetDirectoryName(testLogFile);
			string testDetailDir = Path.GetDirectoryName(testIndexFile);
			
			if (!Directory.Exists(testDetailDir))
				Directory.CreateDirectory(testDetailDir);
			
			// Ensure the old files are cleared
			foreach (string file in Directory.GetFiles(testLogDir + @"\Detail"))
				File.Delete(file);
			...
			LogUtilities.AnalyzeLog(testLogDir);
		...
		public void Test_CreateNewThread()
		{
			...
			XmlDocument doc = LogUtilities.CreateNewThread();
		...
		public void Test_SaveThread()
		{
			...
			LogUtilities.SaveThread(rootDir, threadDoc, threadID);
			
			string threadFile = rootDir + @"\Detail\" + threadID + ".xml";
