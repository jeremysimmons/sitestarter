...
namespace SoftwareMonkeys.SiteStarter.Data
{
	...
	/// </summary>
	public class XmlEntitySchemaEditor
	{
		...
		/*public XmlEntitySchema(string sourceDirectory, string convertedDirectory, string importedDirectory)
		{
			...
			SchemaDirectory = GetSchemaDirectory(applicationPath);
		}*/


		public XmlEntitySchemaEditor(string applicationPath)
		{
		...
		public void Execute()
		{
			LoopThroughSchemaVersions(SchemaDirectory);
		}
		
		public void LoopThroughSchemaVersions(string schemaDirectory)
		{
			foreach (string directory in Directory.GetDirectories(schemaDirectory))
			{
				LoopThroughSchemaFiles(directory);
			}
		}
		
		public void LoopThroughSchemaFiles(string dir)
		{
			foreach (string file in Directory.GetFiles(dir, "*.xml"))
			{
				AnalyzeSchemaFile(file);
			}
		}
		
		public void AnalyzeSchemaFile(string filePath)
		{
		...
		public void AnalyzeSchemaContent(string content)
		{
		...
		public void AnalyzeSchemaNode(XmlNode node)
		{
			switch (node.Name)
			{
				case "RenameProperty":
		...
		{
			foreach (string entityFile in GetEntityFiles(typeName))
			{
				XmlDocument doc = LoadEntityDocument(entityFile);
				
				XmlNode node = ExtractPropertyNode(doc, originalProperty);
				
				node = RenameNode(node, newProperty);
				
				SaveTypeDocument(doc);
			}
		}
		...
		{
			foreach (string entityFile in GetEntityFiles(originalName))
			{
				XmlDocument doc = LoadEntityDocument(entityFile);
				
				XmlDocument renamedDoc = RenameTypeDocument(doc, newName);
				
				SaveTypeDocument(renamedDoc);
			}
		}
		
		public XmlNode ExtractPropertyNode(XmlDocument doc, string propertyName)
		{
			foreach (XmlNode node in doc.DocumentElement.ChildNodes)
			{
				if (node.Name.ToLower() == propertyName.ToLower())
					return node;
			}
		...
		public string[] GetEntityFiles(string typeName)
		{
			List<string> list = new List<string>();
			
			foreach (string directory in Directory.GetDirectories(SourceDirectory))
			{
				string tmpType = EntitiesUtilities.GetShortType(Path.GetFileName(directory));
				
				if (tmpType.ToLower() == typeName.ToLower())
				{
		...
		public XmlDocument LoadEntityDocument(string fileName)
		{
			XmlDocument doc = new XmlDocument();
			doc.Load(fileName);
		...
		public void SaveTypeDocument(XmlDocument doc)
		{
			string id = GetEntityID(doc);
			
			string typeName = GetEntityTypeName(doc);
			
			Type type = EntitiesUtilities.GetType(typeName);
			
			string path = ConvertedDirectory + Path.DirectorySeparatorChar +
				type.FullName + Path.DirectorySeparatorChar +
				id + ".xml";
		...
		public string GetEntityID(XmlDocument doc)
		{
		...
		public string GetEntityTypeName(XmlDocument doc)
		{
			return doc.DocumentElement.Name;
		}
		
		public XmlDocument RenameTypeDocument(XmlDocument doc, string newName)
		{
			...
			//	XmlAttribute attribute = doc.DocumentElement.Attributes[0];
				//doc.DocumentElement.RemoveChild(attribute);
		...
		static public string GetSchemaDirectory(string applicationPath)
		{
			string dir = string.Empty;

			using (LogGroup logGroup = AppLogger.StartGroup("Retrieving the schema directory.", NLog.LogLevel.Debug))
			{
				dir = Path.Combine(applicationPath, @"Schema");
		...
		public void ConvertFileReferences(string fileName, string convertedDirectory, NameValueCollection referenceMappings)
		{
			using (LogGroup logGroup = AppLogger.StartGroup("Converting references from old version to new version in specified file.", NLog.LogLevel.Debug))
			{
				...
				using (FileStream stream = File.OpenRead(fileName))
				{
					...
					{
						throw new Exception("Failed to load data from file: " + fileName);
					}
