...
namespace SoftwareMonkeys.SiteStarter.Diagnostics.Tests
{
	...
	public class LogGroupTests : BaseDiagnosticsTestFixture
	{
		...
		{
			using (LogGroup logGroup = LogGroup.Start("Outer group ", NLog.LogLevel.Debug))
			{
				for (int i = 0; i < 10; i++)
				{
					using (LogGroup logGroup2 = LogGroup.Start("Test group " + i, NLog.LogLevel.Debug))
					{
						if (i > 8)
							break;
					}
		...
		public void Test_Start2()
		{
			...
			using (LogGroup logGroup = LogGroup.Start("Outer group ", NLog.LogLevel.Debug))
			{
				...
				using (LogGroup logGroup2 = LogGroup.Start("Test group", NLog.LogLevel.Debug))
				{
					...
					logGroup3 = LogGroup.Start("Break group", NLog.LogLevel.Debug);
