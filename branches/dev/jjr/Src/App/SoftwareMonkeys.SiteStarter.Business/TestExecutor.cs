...
namespace SoftwareMonkeys.SiteStarter.Business
{
	public class TestExecutor
	{
		...
		/// Runs the tests in the loaded assembly.
		/// </summary>
		public void RunTests(string fixtureName)
		{
			using (LogGroup logGroup = AppLogger.StartGroup("Running unit tests.", NLog.LogLevel.Debug))
			{
				...
				if (TestFixtures.Length > 0)
				{
					...
					{
						if (fixtureName == String.Empty || type.FullName == fixtureName)
						{
					...
					foreach (MethodInfo testMethod in testMethods)
					{
						...
						if (fixtureName == String.Empty || testMethod.DeclaringType.FullName == fixtureName)
						{
							try
							{
